...
action_space = spaces.Discrete(2**3 * 3**2)
...
class BallSortingEnv(gym.Env):
    ...
    ### Episode Termination:
    -   episode ends after 2000 time steps
...
"""
    ...
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(0,3,[n_rows,n_cols], dtype=np.uint8)#[ n_rows, n_cols], dtype=np.uint8)
        ...
        for col in range(n_cols-1):
                ...
                for row in range(n_rows):
                        ...
                        if self.state[row][col] == 0:
                                ...
                                self.reward += self.ok_reward/self.delay_multiplier*(n_cols-col)
                                #else check if incorrect and give penalty
                        else:
                                self.reward += self.nok_reward/self.delay_multiplier*(n_cols-col)
